...
class ChatParser {
	...
	public static function parse($message, $sessionId = 'default') {
		...
		if (preg_match('/^export\s+(extensions?|ringgroups?|dids?|trunks?|cdr|queues?)$/i', $msg, $m)) {
			...
			return ['tool' => 'oc_export', 'params' => ['type' => $type]];
		}

		// ── CDR Analytics ──
		if (preg_match('/^(cdr\s+)?stats$/i', $lower)) {
			return ['tool' => 'oc_get_cdr_stats', 'params' => []];
		}
		if (preg_match('/^(busiest|top)\s+(ext|extensions?)$/i', $lower)) {
			return ['tool' => 'oc_get_busiest_extensions', 'params' => []];
		}
		if (preg_match('/^peak\s+hours?$/i', $lower)) {
			return ['tool' => 'oc_get_peak_hours', 'params' => []];
		}
		if (preg_match('/^failed\s+calls?$/i', $lower)) {
			return ['tool' => 'oc_get_failed_calls', 'params' => []];
		}
		if (preg_match('/^disk\s+space$/i', $lower)) {
			return ['tool' => 'oc_get_disk_space', 'params' => []];
		}
		if (preg_match('/^(sys|system)\s+info$/i', $lower)) {
			return ['tool' => 'oc_get_sys_info', 'params' => []];
		}
		...
		if (preg_match('/^(?:trace|show|get)\s+(?:call\s*)?flow\s+(?:for\s+)?(?:did\s+)?(\S+)$/i', $msg, $m)) {
			...
			return ['tool' => 'oc_trace_call_flow', 'params' => [$key => $val]];
		}
		...
		if (preg_match('/^(create|add|new)\s+(ext|extension)\s+(\d+)\s+(?:(?:with|and)\s+)?(?:voicemail|vm)\s+(?:for\s+|named?\s+)?(.+)$/i', $msg, $m)) {
			...
			self::setPending($sessionId, 'oc_add_extension', $params);
			return ['tool' => 'oc_add_extension', 'params' => $params];
		}
		...
		if (preg_match('/^(clear|remove|cancel|disable)\s+(call\s*)?forward\s*(on\s+|for\s+|from\s+)?(\d+)$/i', $msg, $m)) {
			$params = ['ext' => $m[4]];
			self::setPending($sessionId, 'oc_clear_call_forward', $params);
			return ['tool' => 'oc_clear_call_forward', 'params' => $params];
		}
		if (preg_match('/^(show|get|check)\s+(call\s*)?forward\s*(on\s+|for\s+)?(\d+)$/i', $msg, $m)) {
			return ['tool' => 'oc_get_call_forward', 'params' => ['ext' => $m[4]]];
		}

		// ── DND ──
		if (preg_match('/^(enable|set|turn\s+on)\s+dnd\s+(on\s+|for\s+)?(\d+)$/i', $msg, $m)) {
			$params = ['ext' => $m[3], 'state' => 'on'];
			self::setPending($sessionId, 'oc_toggle_dnd', $params);
			return ['tool' => 'oc_toggle_dnd', 'params' => $params];
		}
		...
		if (preg_match('/^(show|get|check)\s+dnd\s+(on\s+|for\s+)?(\d+)$/i', $msg, $m)) {
			return ['tool' => 'oc_get_dnd', 'params' => ['ext' => $m[3]]];
		}

		// ── Caller ID ──
		if (preg_match('/^set\s+(caller\s*id|cid|outbound\s*cid)\s+(on\s+|for\s+)?(\d+)\s+(?:to\s+)?(\S+)$/i', $msg, $m)) {
			$params = ['ext' => $m[3], 'cid' => $m[4]];
			self::setPending($sessionId, 'oc_set_caller_id', $params);
			return ['tool' => 'oc_set_caller_id', 'params' => $params];
		}
		...
		if (preg_match('/^(enable|set)\s+(recording|record)\s+(on\s+|for\s+)?(\d+)(\s+(?:to\s+)?(always|force))?$/i', $msg, $m)) {
			...
			self::setPending($sessionId, 'oc_set_recording', $params);
			return ['tool' => 'oc_set_recording', 'params' => $params];
		}
		...
		if (preg_match('/^set\s+ring\s*(?:time(?:r|out)?)\s+(on\s+|for\s+)?(\d+)\s+(?:to\s+)?(\d+)s?$/i', $msg, $m)) {
			...
			self::setPending($sessionId, 'oc_set_ringtimer', $params);
			return ['tool' => 'oc_set_ringtimer', 'params' => $params];
		}

		// ── Route Patterns ──
		if (preg_match('/^(show|get)\s+(route\s+)?patterns?\s+(for\s+)?(?:route\s+)?(\d+)$/i', $msg, $m)) {
			return ['tool' => 'oc_get_route_patterns', 'params' => ['id' => $m[4]]];
		}

		// ── All DIDs ──
		if (preg_match('/^(list|show)\s+(all\s+)?dids$/i', $lower)) {
			return ['tool' => 'oc_list_all_dids', 'params' => []];
		}
		...
		if (preg_match('/^(create|generate)\s+(api\s+)?token\s+(?:for\s+)?(\S+)(?:\s+(?:as|with|level)\s+(read|write|admin))?$/i', $msg, $m)) {
			...
			self::setPending($sessionId, 'oc_create_api_token', $params);
			return ['tool' => 'oc_create_api_token', 'params' => $params];
		}
		if (preg_match('/^(list|show)\s+(api\s+)?tokens?$/i', $lower)) {
			return ['tool' => 'oc_list_api_tokens', 'params' => []];
		}
		if (preg_match('/^revoke\s+(api\s+)?token\s+(\d+)$/i', $msg, $m)) {
			$params = ['id' => $m[2]];
			self::setPending($sessionId, 'oc_revoke_api_token', $params);
			return ['tool' => 'oc_revoke_api_token', 'params' => $params];
		}
		if (preg_match('/^delete\s+(api\s+)?token\s+(\d+)$/i', $msg, $m)) {
			$params = ['id' => $m[2]];
			self::setPending($sessionId, 'oc_delete_api_token', $params);
			return ['tool' => 'oc_delete_api_token', 'params' => $params];
		}
		...
		if (preg_match('/^(list|show)\s+(all\s+)?(outbound|outgoing)\s*(routes?)?$/i', $lower)) {
			return ['tool' => 'oc_list_outbound_routes', 'params' => []];
		}
		if (preg_match('/^(show|get)\s+(outbound\s+)?route\s+(\d+)$/i', $msg, $m)) {
			return ['tool' => 'oc_get_outbound_route', 'params' => ['id' => $m[3]]];
		}

		// ── Queues ──
		if (preg_match('/^(list|show)\s+(all\s+)?queues?$/i', $lower)) {
			return ['tool' => 'oc_list_queues', 'params' => []];
		}
		if (preg_match('/^(show|get)\s+queue\s+(\d+)$/i', $msg, $m)) {
			return ['tool' => 'oc_get_queue', 'params' => ['id' => $m[2]]];
		}

		// ── Time Conditions ──
		if (preg_match('/^(list|show)\s+(all\s+)?time\s*conditions?$/i', $lower)) {
			return ['tool' => 'oc_list_time_conditions', 'params' => []];
		}
		if (preg_match('/^toggle\s+time\s*condition\s+(\d+)$/i', $msg, $m)) {
			$params = ['id' => $m[1]];
			self::setPending($sessionId, 'oc_toggle_time_condition', $params);
			return ['tool' => 'oc_toggle_time_condition', 'params' => $params];
		}

		// ── Time Group Detail ──
		if (preg_match('/^(show|get)\s+time\s*group\s+(\d+)$/i', $msg, $m)) {
			return ['tool' => 'oc_get_time_group', 'params' => ['id' => $m[2]]];
		}

		// ── Day/Night ──
		if (preg_match('/^(list|show)\s+(all\s+)?(day\s*\/?\s*night|call\s*flows?)$/i', $lower)) {
			return ['tool' => 'oc_list_daynight', 'params' => []];
		}
		if (preg_match('/^toggle\s+(day\s*\/?\s*night|call\s*flow)\s+(\d+)$/i', $msg, $m)) {
			$params = ['id' => $m[2]];
			self::setPending($sessionId, 'oc_toggle_daynight', $params);
			return ['tool' => 'oc_toggle_daynight', 'params' => $params];
		}
		...
		if (preg_match('/^(show|get)\s+(sip|pjsip)\s*(settings?|config)?$/i', $lower)) {
			return ['tool' => 'oc_get_sip_settings', 'params' => []];
		}

		// ── Reload ──
		if (preg_match('/^(reload|apply|apply\s+config|apply\s+changes)$/i', $lower)) {
			self::setPending($sessionId, 'oc_reload', []);
			return ['tool' => 'oc_reload', 'params' => []];
		}

		// ── Modules ──
		if (preg_match('/^(list|show)\s+(all\s+)?modules?$/i', $lower)) {
			return ['tool' => 'oc_module_list', 'params' => []];
		}
		if (preg_match('/^(module|mod)\s+(status|info|details?)\s+(\S+)$/i', $msg, $m)) {
			return ['tool' => 'oc_module_status', 'params' => ['name' => $m[3]]];
		}
		...
		if (preg_match('/^(?:create|build|make)\s+(?:a\s+)?(?:menu|ivr)\s+(?:on\s+)?(?:ext(?:ension)?\s+)?(\d+)/i', $msg, $m)) {
			...
			self::setPending($sessionId, 'oc_dialplan_apply', $params);
			return ['tool' => 'oc_dialplan_apply', 'params' => $params];
		}
		...
		if (preg_match('/^(show|get)\s+(inbound\s+)?route\s+(.+)$/i', $msg, $m)) {
			return ['tool' => 'oc_get_inbound_route', 'params' => ['extension' => trim($m[3])]];
		}
		if (preg_match('/^(add|create)\s+(inbound\s+)?route\s+(\S+)\s+(?:to|→)\s+(.+)$/i', $msg, $m)) {
			...
			self::setPending($sessionId, 'oc_add_inbound_route', $params);
			return ['tool' => 'oc_add_inbound_route', 'params' => $params];
		}
		if (preg_match('/^(remove|delete)\s+(inbound\s+)?route\s+(\S+)$/i', $msg, $m)) {
			$params = ['extension' => $m[3]];
			self::setPending($sessionId, 'oc_remove_inbound_route', $params);
			return ['tool' => 'oc_remove_inbound_route', 'params' => $params];
		}
		...
		if (preg_match('/^(add|create)\s+(ring\s*group|rg)\s+(\d+)\s+(?:with|members?|ext)\s+(.+?)(?:\s+strategy\s+(\w+))?$/i', $msg, $m)) {
			...
			self::setPending($sessionId, 'oc_add_ringgroup', $params);
			return ['tool' => 'oc_add_ringgroup', 'params' => $params];
		}
		if (preg_match('/^delete\s+(ring\s*group|rg)\s+(\d+)$/i', $msg, $m)) {
			$params = ['id' => $m[2]];
			self::setPending($sessionId, 'oc_delete_ringgroup', $params);
			return ['tool' => 'oc_delete_ringgroup', 'params' => $params];
		}
		...
		if (preg_match('/^upgrade\s+all\s+modules$/i', $lower)) {
			$params = ['name' => 'all'];
			self::setPending($sessionId, 'oc_module_upgrade', $params);
			return ['tool' => 'oc_module_upgrade', 'params' => $params];
		}
		if (preg_match('/^(need|check)\s*reload$/i', $lower)) {
			return ['tool' => 'oc_need_reload', 'params' => []];
		}

		// ── Voicemail Enable/Disable ──
		if (preg_match('/^enable\s+voicemail\s+(on\s+|for\s+)?(\d+)$/i', $msg, $m)) {
			$params = ['ext' => $m[2]];
			self::setPending($sessionId, 'oc_enable_voicemail', $params);
			return ['tool' => 'oc_enable_voicemail', 'params' => $params];
		}
		if (preg_match('/^disable\s+voicemail\s+(on\s+|for\s+)?(\d+)$/i', $msg, $m)) {
			$params = ['ext' => $m[2]];
			self::setPending($sessionId, 'oc_disable_voicemail', $params);
			return ['tool' => 'oc_disable_voicemail', 'params' => $params];
		}

		// ── Advanced Settings ──
		if (preg_match('/^(show|get)\s+(advanced\s+)?setting\s+(\S+)$/i', $msg, $m)) {
			return ['tool' => 'oc_get_advanced_setting', 'params' => ['key' => $m[3]]];
		}
		if (preg_match('/^list\s+(advanced\s+)?settings$/i', $lower)) {
			return ['tool' => 'oc_get_advanced_setting', 'params' => ['key' => 'list']];
		}
		if (preg_match('/^set\s+(advanced\s+)?setting\s+(\S+)\s+(?:to\s+)?(.+)$/i', $msg, $m)) {
			$params = ['key' => $m[2], 'value' => trim($m[3])];
			self::setPending($sessionId, 'oc_set_advanced_setting', $params);
			return ['tool' => 'oc_set_advanced_setting', 'params' => $params];
		}

		// ── Firewall Network ──
		if (preg_match('/^(add|allow)\s+(\S+)\s+(?:to\s+)?(?:firewall\s+)?zone\s+(\w+)$/i', $msg, $m)) {
			$params = ['network' => $m[2], 'zone' => $m[3]];
			self::setPending($sessionId, 'oc_firewall_add_network', $params);
			return ['tool' => 'oc_firewall_add_network', 'params' => $params];
		}

		// ── Backups ──
		if (preg_match('/^list\s+backups?$/i', $lower)) {
			return ['tool' => 'oc_list_backups', 'params' => []];
		}
		if (preg_match('/^(show|get)\s+backup\s+(\S+)$/i', $msg, $m)) {
			return ['tool' => 'oc_get_backup', 'params' => ['id' => $m[2]]];
		}

		// ── Certificates ──
		if (preg_match('/^list\s+(cert|certificate|ssl|tls)s?$/i', $lower)) {
			return ['tool' => 'oc_list_certificates', 'params' => []];
		}

		// ── Filestores ──
		if (preg_match('/^list\s+(filestore|storage)s?$/i', $lower)) {
			return ['tool' => 'oc_list_filestores', 'params' => []];
		}

		// ── PM2 ──
		if (preg_match('/^(show|list|get)\s+(pm2|services?|processes?)$/i', $lower)) {
			return ['tool' => 'oc_get_pm2_status', 'params' => []];
		}

		// ── License ──
		if (preg_match('/^(show|get)\s+(license|licence|activation)$/i', $lower)) {
			return ['tool' => 'oc_get_license_info', 'params' => []];
		}
		if (preg_match('/^(update|refresh|renew)\s+(activation|license|licence)$/i', $lower)) {
			self::setPending($sessionId, 'oc_update_activation', []);
			return ['tool' => 'oc_update_activation', 'params' => []];
		}

		// ── SIP Diagnostics ──
		if (preg_match('/^diagnose\s+(ext(ension)?)\s+(\d+)$/i', $msg, $m)) {
			return ['tool' => 'oc_diagnose_extension', 'params' => ['ext' => $m[3]]];
		}
		if (preg_match('/^(troubleshoot|debug)\s+(ext(ension)?)?\s*(\d+)$/i', $msg, $m)) {
			return ['tool' => 'oc_diagnose_extension', 'params' => ['ext' => $m[4]]];
		}
		if (preg_match('/^(why\s+can.?t|what.?s\s+wrong\s+with)\s+(ext(ension)?)?\s*(\d+)/i', $msg, $m)) {
			return ['tool' => 'oc_diagnose_extension', 'params' => ['ext' => $m[4]]];
		}
		if (preg_match('/^diagnose\s+trunk\s+(\d+)$/i', $msg, $m)) {
			return ['tool' => 'oc_diagnose_trunk', 'params' => ['id' => $m[1]]];
		}
		if (preg_match('/^(troubleshoot|debug)\s+trunk\s+(\d+)$/i', $msg, $m)) {
			return ['tool' => 'oc_diagnose_trunk', 'params' => ['id' => $m[2]]];
		}
		if (preg_match('/^(show|get)\s+endpoint\s+(?:details?\s+)?([a-zA-Z0-9_\-]+)$/i', $msg, $m)) {
			return ['tool' => 'oc_pjsip_endpoint_details', 'params' => ['endpoint' => $m[2]]];
		}
		if (preg_match('/^endpoint\s+(?:details?\s+)?([a-zA-Z0-9_\-]+)$/i', $msg, $m)) {
			if (strtolower($m[1]) !== 'details' && strtolower($m[1]) !== 'detail') {
				return ['tool' => 'oc_pjsip_endpoint_details', 'params' => ['endpoint' => $m[1]]];
			}
		}
		if (preg_match('/^(show\s+)?(sip|pjsip)\s+channels?$/i', $lower)) {
			return ['tool' => 'oc_pjsip_show_channels', 'params' => []];
		}
		if (preg_match('/^(show\s+)?(sip|pjsip)\s+channels?\s+(for\s+)?(\S+)$/i', $msg, $m)) {
			return ['tool' => 'oc_pjsip_show_channels', 'params' => ['endpoint' => $m[4]]];
		}
		if (preg_match('/^(start|begin)\s+(sip\s+)?trace(\s+(\d+)s?)?$/i', $msg, $m)) {
			$params = ['action' => 'start'];
			if (!empty($m[4])) $params['duration'] = (int)$m[4];
			return ['tool' => 'oc_sip_trace', 'params' => $params];
		}
		if (preg_match('/^stop\s+(sip\s+)?trace$/i', $lower)) {
			return ['tool' => 'oc_sip_trace', 'params' => ['action' => 'stop']];
		}
		if (preg_match('/^(sip\s+)?trace\s+status$/i', $lower)) {
			return ['tool' => 'oc_sip_trace', 'params' => ['action' => 'status']];
		}

		// ── PJSIP Diagnostics ──
		if (preg_match('/^ping\s+(\S+)$/i', $msg, $m)) {
			return ['tool' => 'oc_pjsip_qualify', 'params' => ['ext' => $m[1]]];
		}
		if (preg_match('/^(show\s+)?registrations?$/i', $lower)) {
			return ['tool' => 'oc_pjsip_registrations', 'params' => []];
		}
		if (preg_match('/^extension\s+states?$/i', $lower)) {
			return ['tool' => 'oc_extension_states', 'params' => []];
		}
		if (preg_match('/^rotate\s+logs?$/i', $lower)) {
			return ['tool' => 'oc_rotate_logs', 'params' => []];
		}

		// ── Live Call Control ──
		if (preg_match('/^call\s+(\d+)\s+(?:to\s+)(\S+)$/i', $msg, $m)) {
			$params = ['from' => $m[1], 'to' => $m[2]];
			self::setPending($sessionId, 'oc_originate_call', $params);
			return ['tool' => 'oc_originate_call', 'params' => $params];
		}
		if (preg_match('/^hangup\s+(\S+)$/i', $msg, $m)) {
			$params = ['channel' => $m[1]];
			self::setPending($sessionId, 'oc_hangup_call', $params);
			return ['tool' => 'oc_hangup_call', 'params' => $params];
		}
		if (preg_match('/^transfer\s+(\S+)\s+(?:to\s+)(\S+)$/i', $msg, $m)) {
			$params = ['channel' => $m[1], 'ext' => $m[2]];
			self::setPending($sessionId, 'oc_transfer_call', $params);
			return ['tool' => 'oc_transfer_call', 'params' => $params];
		}
		if (preg_match('/^park\s+(\S+)$/i', $msg, $m)) {
			$params = ['channel' => $m[1]];
			self::setPending($sessionId, 'oc_park_call', $params);
			return ['tool' => 'oc_park_call', 'params' => $params];
		}
		if (preg_match('/^record\s+(\S+)$/i', $msg, $m)) {
			$params = ['channel' => $m[1]];
			self::setPending($sessionId, 'oc_monitor_call', $params);
			return ['tool' => 'oc_monitor_call', 'params' => $params];
		}
		if (preg_match('/^stop\s+record(ing)?\s+(\S+)$/i', $msg, $m)) {
			$params = ['channel' => $m[2]];
			self::setPending($sessionId, 'oc_stop_monitor_call', $params);
			return ['tool' => 'oc_stop_monitor_call', 'params' => $params];
		}
		if (preg_match('/^mute\s+(\S+)$/i', $msg, $m)) {
			$params = ['channel' => $m[1], 'state' => 'on'];
			self::setPending($sessionId, 'oc_mute_call', $params);
			return ['tool' => 'oc_mute_call', 'params' => $params];
		}
		...
		if (preg_match('/^add\s+(\d+)\s+to\s+queue\s+(\d+)$/i', $msg, $m)) {
			...
			self::setPending($sessionId, 'oc_queue_add_agent', $params);
			return ['tool' => 'oc_queue_add_agent', 'params' => $params];
		}
		if (preg_match('/^remove\s+(\d+)\s+from\s+queue\s+(\d+)$/i', $msg, $m)) {
			$params = ['ext' => $m[1], 'queue' => $m[2]];
			self::setPending($sessionId, 'oc_queue_remove_agent', $params);
			return ['tool' => 'oc_queue_remove_agent', 'params' => $params];
		}
		if (preg_match('/^pause\s+(\d+)\s+in\s+queue\s+(\d+)$/i', $msg, $m)) {
			$params = ['ext' => $m[1], 'queue' => $m[2], 'paused' => true];
			self::setPending($sessionId, 'oc_queue_pause_agent', $params);
			return ['tool' => 'oc_queue_pause_agent', 'params' => $params];
		}
		...
		if (preg_match('/^queue\s+status(?:\s+(\d+))?$/i', $msg, $m)) {
			...
			return ['tool' => 'oc_queue_status', 'params' => $params];
		}

		// ── Conference Control ──
		if (preg_match('/^who.s\s+in\s+conference\s+(\d+)$/i', $msg, $m)) {
			return ['tool' => 'oc_conference_participants', 'params' => ['id' => $m[1]]];
		}
		if (preg_match('/^kick\s+(\S+)\s+from\s+conference\s+(\d+)$/i', $msg, $m)) {
			$params = ['id' => $m[2], 'channel' => $m[1]];
			self::setPending($sessionId, 'oc_conference_kick', $params);
			return ['tool' => 'oc_conference_kick', 'params' => $params];
		}
		if (preg_match('/^(mute|unmute)\s+(\S+)\s+in\s+conference\s+(\d+)$/i', $msg, $m)) {
			$params = ['id' => $m[3], 'channel' => $m[2], 'state' => strtolower($m[1]) === 'mute' ? 'on' : 'off'];
			self::setPending($sessionId, 'oc_conference_mute', $params);
			return ['tool' => 'oc_conference_mute', 'params' => $params];
		}
		if (preg_match('/^lock\s+conference\s+(\d+)$/i', $msg, $m)) {
			$params = ['id' => $m[1], 'state' => 'lock'];
			self::setPending($sessionId, 'oc_conference_lock', $params);
			return ['tool' => 'oc_conference_lock', 'params' => $params];
		}
		...
		if (preg_match('/^set\s+external\s*ip\s+(?:to\s+)?(\S+)$/i', $msg, $m)) {
			...
			self::setPending($sessionId, 'oc_update_sip_nat', $params);
			return ['tool' => 'oc_update_sip_nat', 'params' => $params];
		}

		// ── fwconsole ──
		if (preg_match('/^fwconsole\s+(.+)$/i', $msg, $m)) {
			$params = ['args' => trim($m[1])];
			self::setPending($sessionId, 'oc_fwconsole', $params);
			return ['tool' => 'oc_fwconsole', 'params' => $params];
		}
		...
		if (preg_match('/^list\s+permissions?$/i', $lower)) {
			return ['tool' => 'oc_list_permissions', 'params' => []];
		}
		if (preg_match('/^set\s+permission\s+(\S+)\s+(?:to\s+)?(read|write|admin)$/i', $msg, $m)) {
			$params = ['username' => $m[1], 'level' => strtolower($m[2])];
			self::setPending($sessionId, 'oc_set_permission', $params);
			return ['tool' => 'oc_set_permission', 'params' => $params];
		}


		// ── Start/Stop/Restart ──
		if (preg_match('/^start\s+(freepbx|pbx|asterisk|services?)$/i', $lower)) {
			self::setPending($sessionId, 'oc_start', []);
			return ['tool' => 'oc_start', 'params' => []];
		}
		if (preg_match('/^stop\s+(freepbx|pbx|asterisk|services?)$/i', $lower)) {
			self::setPending($sessionId, 'oc_stop', []);
			return ['tool' => 'oc_stop', 'params' => []];
		}
		if (preg_match('/^restart\s+(freepbx|pbx|asterisk|services?)$/i', $lower)) {
			self::setPending($sessionId, 'oc_restart', []);
			return ['tool' => 'oc_restart', 'params' => []];
		}

		// ── Trunk enable/disable ──
		if (preg_match('/^enable\s+trunk\s+(\d+)$/i', $msg, $m)) {
			$params = ['id' => $m[1]];
			self::setPending($sessionId, 'oc_enable_trunk', $params);
			return ['tool' => 'oc_enable_trunk', 'params' => $params];
		}
		if (preg_match('/^disable\s+trunk\s+(\d+)$/i', $msg, $m)) {
			$params = ['id' => $m[1]];
			self::setPending($sessionId, 'oc_disable_trunk', $params);
			return ['tool' => 'oc_disable_trunk', 'params' => $params];
		}

		// ── Validate ──
		if (preg_match('/^validate|^security\s+scan/i', $lower)) {
			return ['tool' => 'oc_validate', 'params' => []];
		}

		// ── Chown ──
		if (preg_match('/^(fix\s+permissions|chown)$/i', $lower)) {
			self::setPending($sessionId, 'oc_chown', []);
			return ['tool' => 'oc_chown', 'params' => []];
		}

		// ── External IP ──
		if (preg_match('/^(external\s+ip|public\s+ip|my\s+ip|what.s\s+my\s+ip)$/i', $lower)) {
			return ['tool' => 'oc_get_external_ip', 'params' => []];
		}

		// ── Notifications ──
		if (preg_match('/^(list|show)\s+notifications?$/i', $lower)) {
			return ['tool' => 'oc_list_notifications', 'params' => []];
		}
		if (preg_match('/^show\s+notification\s+(\S+)$/i', $msg, $m)) {
			return ['tool' => 'oc_list_notifications', 'params' => ['id' => $m[1]]];
		}

		// ── Show any dialplan context ──
		if (preg_match('/^(show|get)\s+asterisk\s+context\s+(\S+)$/i', $msg, $m)) {
			return ['tool' => 'oc_show_context', 'params' => ['name' => $m[2]]];
		}

		// ── Sounds ──
		if (preg_match('/^(list|show)\s+(sound|language)\s*packs?$/i', $lower)) {
			return ['tool' => 'oc_list_sounds', 'params' => []];
		}

		// ── Sync userman ──
		if (preg_match('/^sync\s+(userman|users?|directory)$/i', $lower)) {
			self::setPending($sessionId, 'oc_sync_userman', []);
			return ['tool' => 'oc_sync_userman', 'params' => []];
		}

		// ── System update ──
		if (preg_match('/^(system\s+update|update\s+system)$/i', $lower)) {
			self::setPending($sessionId, 'oc_system_update', []);
			return ['tool' => 'oc_system_update', 'params' => []];
		}

		// ── PM2 manage ──
		if (preg_match('/^(restart|stop)\s+(pm2|service|process)\s+(\S+)$/i', $msg, $m)) {
			$params = ['action' => strtolower($m[1]), 'name' => $m[3]];
			self::setPending($sessionId, 'oc_pm2_manage', $params);
			return ['tool' => 'oc_pm2_manage', 'params' => $params];
		}

		// ── Update certificates ──
		if (preg_match('/^(update|renew)\s+(all\s+)?(cert|certificate|ssl)s?$/i', $lower)) {
			self::setPending($sessionId, 'oc_update_certificates', []);
			return ['tool' => 'oc_update_certificates', 'params' => []];
		}

		// ── Shorthand: just a number = get extension ──
		if (preg_match('/^(\d{3,6})$/', $msg, $m)) {
			return ['tool' => 'oc_get_extension', 'params' => ['ext' => $m[1]]];
		}
